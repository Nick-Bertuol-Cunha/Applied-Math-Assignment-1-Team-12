...
function convergence_analysis(solver_flag, fun, x_guess0, guess_list1, guess_list2, filter_list)
    ...
    roots = zeros(num_trials, 1);
    iterations = zeros(num_trials, 1);
    
    % Loop through each guess in guess_list1
    for i = 1:num_trials
        if solver_flag == 1 % Bisection
            roots(i) = bisection_solver(fun, guess_list1(i), guess_list2(i));
        elseif solver_flag == 2 % Newton
            [roots(i), xi_values] = newton_solver(fun, guess_list1(i));
            iterations(i) = length(xi_values);
        elseif solver_flag == 3 % Secant
            roots(i) = secant_solver(fun, guess_list1(i), guess_list2(i));
        elseif solver_flag == 4 % fzero
            roots(i) = fzero(fun, guess_list1(i));
        end
    end
    
    % Filter results based on filter_list
    filtered_roots = roots(ismember(roots, filter_list));
    
    % Display results
    fprintf('Roots found:\n');
    disp(filtered_roots);
    fprintf('Number of iterations for each trial:\n');
    disp(iterations);
end
