...
test_func01 = fun;
test_derivative01 = @(x) approximate_derivative(fun, x);
...
for t = 1:numel(x0_list)
    rec = input_recorder();
    f_rec = rec.generate_recorder_fun(test_func01); 
    try
        for i = 1:num_trials
            if solver_flag == 1 % Bisection
                x_star = bisection_solver(f_rec,x0_list(t),guess_list2(t));

            elseif solver_flag == 2 % Newton
                x_star = newton_solver(f_rec,x0_list(t));
            
            elseif solver_flag == 3 % Secant
                x_star = secant_solver(f_rec,x0_list(t),guess_list2(t));
            elseif solver_flag == 4 % fzero
                x_star = fzero(f_rec,x0_list(t));
...
y_regression=[];
for n=1:length(trial_id)
    if err_n(n)>filter_list(1) && err_n(n)<filter_list(2) && ...
        err_np1(n)>filter_list(3) && err_np1(n)<filter_list(4) && ...
        trial_id(n)>filter_list(5)
        x_regression(end+1) = err_n(n);
        y_regression(end+1) = err_np1(n);
    end
    n=n+1;
